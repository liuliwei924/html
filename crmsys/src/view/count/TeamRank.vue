<template>
  <div class="content-box">
    <div class="header__title">门店分队排名统计</div>
    <!-- 条件搜索栏 -->
    ...
    <el-table
      ...
      <el-table-column
        ...
        label="今日新增创收(元)"
        :render-header="(h, options) => {
        return renderHeader(h, options, '今日回款总额')}">
      </el-table-column>
      <el-table-column
        min-width="150"      
    ...
    <form :action="exportPath" method="post" id="JS-form-export" target="_blank" style="display:none">
          ...
          <input type="text" name="exportType" value="teamRank">
    </form>
  </div>
</template>
...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('teamRank')
    return {
  ...
  methods: {
    ...
    searchHandle () {
      ...
      // 缓存查询条件
      this.$ajax({
        url: '/store/account/dataset/teamRank/queryOrgTeamRank',
      ...
      this.queryOrgSumRank()
    },
    // 查询门店排名求和
    queryOrgSumRank () {
      // 缓存查询条件
      this.$ajax({
        url: '/store/account/dataset/teamRank/queryOrgSumRank',
        data: this.searchForm,
        success: data => {
