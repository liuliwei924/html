<template>
  <div class="content-box">
    <div class="header__title">反馈数据</div>
    <div class="header__search clearfix">
      <el-form :model="searchForm" ref="searchForm">
        ...
        <el-form-item label="反馈ID" prop="succId">
          <el-input v-model="searchForm.succId" placeholder="请输入反馈ID"></el-input>
        </el-form-item>
        ...
        <el-button :loading="loading" @click="settleAmount">结算</el-button>
      </el-form>
    </div>
    <el-table class="content__table" :data="tableData" border @selection-change="selectChange">
      ...
      <el-table-column prop="importTime" label="导入时间" align="center" min-width="105">
      </el-table-column>
       <el-table-column prop="backStatus" label="反馈状态" align="center" min-width="65">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.backStatus===0">待审核</el-tag>
          <el-tag v-if="scope.row.backStatus===1" type="success">申请成功</el-tag>
        </template>
      </el-table-column>
       <el-table-column prop="matchStatus" label="匹配状态" min-width="60 ">
        <template slot-scope="scope">
...
<script>
import FeedbackMatch from '@/components/xj/CreditFeedbackMatch'
export default {
  name: 'use-manager',
  data () {
    ...
    return {
      ...
      excelImportPath: this.$baseURL + 'store/account/xj/creditFeedBack/feedbackImport',
      excelExportPath: this.$baseURL + 'store/account/xj/creditFeedBack/feedbackExport',
      excelForm: {
        signId: signId
      },
      loading: false,
  ...
  methods: {
    ...
    download () {
      let signId = JSON.parse(this.$localStorage('signId')).v
      window.open(this.$baseURL + 'store/account/xj/creditFeedBack/getFeedbackTemp?signId=' + signId)
    },
    // 文件上传之前处理
    beforeUp () {
      this.loading = true
    ...
    searchHandle () {
      this.$ajax({
        url: '/store/account/xj/creditFeedBack/queryFeedbackList',
    ...
    settleAmount () {
      this.$ajax({
        url: '/store/account/xj/creditFeedBack/settleAmount',
