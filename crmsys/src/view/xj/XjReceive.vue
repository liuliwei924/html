<template>
  <div class="content-box">
    <div class="header__title">天狗币领取记录</div>
...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('xjReceive')
    return {
      tableData: [],
      totalRecord: 1,
      searchForm: {
  ...
  methods: {
    ...
    searchHandle () {
      this.tableData = []
       // 缓存查询条件
      this.$setStore('xjReceive', this.searchForm, ['startDate', 'endDate'])
      this.$ajax({
        url: '/store/account/xj/xjReceive/queryReceiveRecord',
