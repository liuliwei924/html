  ...
  <div class="content-box">
    ...
    <el-table class="content__table" :data="tableData" border @selection-change="selectChange">
      ...
      <el-table-column prop="customerId" label="客户编号" align="center" width="90">
       ...
       <el-table-column prop="nickName" label="用户昵称" align="center" width="90">
      </el-table-column>
      <el-table-column prop="realName" label="真实姓名" align="center" width="90">
      </el-table-column>
      ...
      <el-table-column prop="transactionId" label="支付交易号" align="center">
      </el-table-column>
    ...
    <xjWithdraw-audit v-model="auditShow" :ids="ids" @change="auditChange" />
    <xjSend-reward-amt v-model="rewardAmtShow" :withdrawIds="withdrawIds" @change="rewardAmtChange" />
  </div>
</template>


<script>
import XjWithdrawAudit from '@/components/xj/XjWithdrawAudit'
import XjSendRewardAmt from '@/components/xj/XjSendRewardAmt'
export default {
  name: 'xj-withdraw',
  data () {
    return {
  ...
  methods: {
    ...
    queryRewardAmt () {
      ...
      this.$ajax({
        url: '/store/account/xj/xjWithdraw/querySendStatus',
        data: {withdrawIds, queryType: 'xj'},
        loading: true,
        success: data => {
          this.searchHandle()
        }
    ...
    cancelHandle (withdrawId) {
      this.$alert('您是否确定要取消提现申请?', '提示', {
        ...
        callback: action => {
          if (action === 'confirm') {
            this.$ajax({
              url: '/store/account/xj/xjWithdraw/auditNoPass',
              data: {
                withdrawId
              },
              success: data => {
    ...
    searchHandle () {
      this.$ajax({
        url: '/store/account/xj/xjWithdraw/queryWithdrawList',
