<template>
  <div class="content-box">
    <div class="header__title">待放款</div>
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button type="text" class="table__danger" size="small" @click="checkLend(scope.row.applyId)">放款</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
<script>
export default {
  name: 'bank-waiting-loan',
  data () {
    let store = this.$getStore('bankWaitingLoan')
    return {
      searchForm: {
  ...
  methods: {
    // 查询处理
    searchHandle (params) {
      if (params === 'store') this.$setStore('bankWaitingLoan', this.searchForm)
      this.$ajax({
        url: '/crmsys/account/busi/waitLend/queryList',
    ...
    checkLend (applyId) {
      this.$ajax({
        url: '/crmsys/account/busi/waitLend/checkLend',
