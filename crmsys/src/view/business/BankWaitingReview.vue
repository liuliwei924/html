<template>
  <div class="content-box">
    <div class="header__title">待复核</div>
    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button type="text" class="table__danger" size="small" @click="viewVerify(scope.row.applyId, 'bankWaitReview')">复核</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
<script>
export default {
  name: 'back-waiting-review',
  data () {
    let store = this.$getStore('backWaitingReview')
    return {
      searchForm: {
  ...
  methods: {
    // 查询处理
    searchHandle (params) {
      if (params === 'store') this.$setStore('backWaitingReview', this.searchForm)
      this.$ajax({
        url: '/crmsys/account/busi/bankWaitReview/queryList',
    ...
    viewVerify (applyId, type) {
      this.$router.push({path: 'busiVerify', query: {type: type, applyId: applyId}})
    }
  }
}
</script>