  ...
  <div class="content-box">
    <div class="header__title">已预约上门客户</div>
    <!-- 条件搜索 -->
...
export default {
  name: 'pre-visit-cust',
  data () {
    let store = this.$getStore('preVisitCust')
    return {
      searchForm: {
  ...
  methods: {
    ...
    // 查询处理
    searchHandle (params) {
      if (params === 'store') this.$setStore('preVisitCust', this.searchForm)
      this.$ajax({
        url: '/crmsys/account/busi/reservation/queryList',
