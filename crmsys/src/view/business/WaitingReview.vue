    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button type="text" class="table__danger" size="small" @click="viewVerify(scope.row.applyId, 'waitReview')">复核</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  name: 'waiting-review',
  data () {
    let store = this.$getStore('waitingReview')
    return {
      searchForm: {
  ...
  methods: {
    // 查询处理
    searchHandle (params) {
      if (params === 'store') this.$setStore('waitingReview', this.searchForm)
      this.$ajax({
        url: '/crmsys/account/busi/waitReview/queryList',
