  <template>
    <div class="content-box">
      ...
      <el-form :model="searchForm" ref="searchForm">
        ...
        <el-form-item label="海报类型" prop="type">
          <el-select v-model="searchForm.type" clearable placeholder="请选择海报类型">
            <el-option label="信用卡海报" value="1"></el-option>
            <el-option label="推广海报" value="2"></el-option>
            <el-option label="专属海报" value="3"></el-option>
            <el-option label="邀请办卡" value="4"></el-option>
            <el-option label="贷款海报" value="6"></el-option>
          </el-select>
        </el-form-item>
      ...
      <el-table class="content__table" :data="tableData" border>
        <el-table-column prop="type" label="海报类型" align="center" min-width="30">
          <template slot-scope="scope">
            <span v-if="scope.row.type===1">信用卡海报</span>
            <span v-if="scope.row.type===2">推广海报</span>
            <span v-if="scope.row.type===3">专属海报</span>
            <span v-if="scope.row.type===4">邀请办卡</span>
            <span v-if="scope.row.type===6">贷款海报</span>
          </template>
        </el-table-column>
        <el-table-column
          prop="prodName"
        ...
        <el-table-column
         prop="isDtlImage"
          label="产品详情显示" align="center">
          <template slot-scope="scope">
            <el-tag v-if ="scope.row.isDtlImage===0" type="danger">不显示</el-tag>
            <el-tag v-if ="scope.row.isDtlImage===1" type="success">显示</el-tag>
          </template>
        </el-table-column>
      ...
      <fx-poster-add v-model="addShow" @change="addChange" :prodList="prodList"/>
      <fx-poster-edit v-model="editShow" :fxPosterData="fxPosterData" @change="editChange" :prodList="prodList"/>
...
<script>
import FxPosterAdd from '@/components/fx/FxPosterAdd'
import FxPosterEdit from '@/components/fx/FxPosterEdit'
export default {
  name: 'fx-poster-cfg',
  data () {
    return {
  ...
  methods: {
    ...
    searchHandle () {
      this.$ajax({
        url: '/store/account/fx/fxPoster/queryPosterList',
