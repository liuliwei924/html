<template>
  <div class="content-box">
    <div class="header__title">推荐提现记录</div>
    <div class="header__search clearfix">
      <el-form :model="searchForm" ref="searchForm">
        <!-- 查询条件 -->
        <el-form-item label="姓名/手机号" prop="realName">
        ...
        <el-form-item label="提现类型" prop="withdrawType">
          <el-select v-model="searchForm.withdrawType" clearable placeholder="提现类型">
        ...
        <el-button icon="plus" @click="queryTjRewardAmt()">交易查询</el-button>
      </el-form>
    </div>
    <el-table class="content__table" :data="tableData" border @selection-change="selectChange">
      ...
      <el-table-column prop="realName" label="姓名" align="center" width="110">
      </el-table-column>
      ...
      <el-table-column prop="cardCode" label="银行卡号" align="center">
      </el-table-column>
      <el-table-column prop="amount" label="提现金额" align="center">
      </el-table-column>
      <el-table-column prop="poundage" label="提现手续费" align="center">
      </el-table-column>
      ...
      <el-table-column prop="status" label="状态" align="center">
        <template slot-scope="scope">
          ...
          <span v-if ="scope.row.status===2">提现取消</span>
          <span v-if ="scope.row.status===3">交易中</span>
          <span v-if ="scope.row.status===4">提现成功</span>
          <span v-if ="scope.row.status===5">失败</span>
        </template>
      </el-table-column>
       <el-table-column prop="withdrawType" label="提现类型" align="center">
        <template slot-scope="scope">
          <span v-if ="scope.row.withdrawType==='tj'">推荐贷款</span>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="checkBy" label="发放人" align="center">
      </el-table-column>
      <el-table-column prop="checkTime" label="发放时间" align="center">
      </el-table-column>
    ...
    <tj-withdraw-audit v-model="auditShow" :ids="ids" @change="auditChange" />
    <tj-send-reward-amt v-model="rewardAmtShow" :withdrawIds="withdrawIds" @change="rewardAmtChange" />
  </div>
</template>


<script>
import TjWithdrawAudit from '@/components/fx/TjWithdrawAudit'
import TjSendRewardAmt from '@/components/fx/TjSendRewardAmt'
export default {
  name: 'fx-cust',
  data () {
    return {
  ...
  methods: {
    ...
    cancelHandle (withdrawId) {
      this.$alert('您是否确定要取消提现申请?', '提示', {
        ...
        callback: action => {
          if (action === 'confirm') {
            this.$ajax({
              url: '/store/account/fx/tjWithdrawtList/tjAuditNoPass',
              data: {
                withdrawId
              },
              success: data => {
    ...
    searchHandle () {
      this.$ajax({
        url: '/store/account/fx/tjWithdrawtList/queryTjWithdrawList',
