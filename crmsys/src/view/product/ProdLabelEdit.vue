    ...
    getProdInfo () {
      this.$ajax({
        ...
        success: data => {
          ...
          let prodNameArray = form.prodNameArray.split(',')
          let prodId = form.prodId.split(',')
          let checkProdArr = []
          for (let i = 0; i < prodNameArray.length; i++) {
            let obj = {prodName: prodNameArray[i], prodId: prodId[i]}
            checkProdArr.push(obj)
          }
          this.prodArray = checkProdArr
        }
      })
    },
    // 获取产品信息
