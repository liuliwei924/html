<template>
  <div class="content-box">
    <div class="header__title">结案列表</div>
    <!-- 条件搜索栏 -->
    ...
    <el-table
      ...
      <el-table-column
        label="签约状态"
        min-width="180">
        <template slot-scope="scope">{{signStatus[scope.row.signStatus]}}</template>
      </el-table-column>
      <el-table-column
        prop="failureCause"
        label="签约失败原因"
...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('signEnd')
    return {
  ...
  methods: {
    ...
    searchHandle () {
      ...
      // 缓存查询条件
      this.$setStore('signEnd', this.searchForm, ['startDate', 'endDate'])
      this.$ajax({
        url: '/store/account/work/signEnd/querySignEnd',
