<template>
  <div class="content-box">
    <div class="header__title">未上门待签约</div>
    <!-- 条件搜索栏 -->
...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('notWaitSignOrder')
    return {
  ...
  methods: {
    ...
    searchHandle () {
      this.tableData = []
      // 缓存查询条件
      this.$setStore('notWaitSignOrder', this.searchForm, ['startHandleDate', 'endHandleDate'])
      this.$ajax({
        url: '/store/account/work/notWaitSignOrder/queryNotWaitSignOrder',
