<template>
  <div class="content-box">
    <div class="header__title">新申请</div>
    <!-- 条件搜索栏 -->
...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('waitDeals')
    return {
      signId: '',
      // 导出
      exportSearchParams: {},
      exportForm: {
        exportType: 'storeWaitDeals',
        exportParams: ''
      },
      defileName: 'newApply',
      searchForm: {
  ...
  methods: {
    ...
    searchHandle () {
      ...
      // 缓存查询条件
      this.$setStore('waitDeals', this.searchForm, ['startAllotDate', 'endAllotDate'])
      this.$ajax({
        url: '/store/account/work/waitDeal/queryNewApplay',
