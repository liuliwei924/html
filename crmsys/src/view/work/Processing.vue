<template>
  <div class="content-box">
    <div class="header__title">处理中</div>
    <!-- 条件搜索栏 -->
...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('processing')
    return {
  ...
  methods: {
    ...
    searchHandle () {
      // 缓存查询条件
      this.$setStore('processing', this.searchForm, ['startHandleDate', 'endHandleDate'])
      this.$ajax({
        url: '/store/account/work/processing/queryProcessList',
