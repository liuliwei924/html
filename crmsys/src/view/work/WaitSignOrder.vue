<template>
  <div class="content-box">
    <div class="header__title">已上门待签约</div>
    <!-- 条件搜索栏 -->
...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('waitSignOrder')
    return {
      searchForm: {
        orgId: store['orgId'] || '',
  ...
  methods: {
    ...
    searchHandle () {
      this.tableData = []
      // 缓存查询条件
      this.$setStore('waitSignOrder', this.searchForm, ['startHandleDate', 'endHandleDate'])
      this.$ajax({
        url: '/store/account/work/waitSignOrder/queryWaitSignOrder',
