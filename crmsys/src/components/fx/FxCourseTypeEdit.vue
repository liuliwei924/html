<template>
  <el-dialog class="edit-dialog" title="编辑课程类型" v-model="visible">
...
<script>
export default {
  name: 'fxCourseType-edit',
  props: {
    value: Boolean,
    fxCourseTypeData: Object
  },
  data () {
    return {
      visible: this.value,
  ...
  methods: {
    ...
    confirm () {
      this.$refs['form'].validate(valid => {
        if (valid) {
          this.$ajax({
            url: '/store/account/fx/fxCourseConf/updateCourTypeInfo',
            data: this.form,
            success: data => {
