        ...
        <el-table-column prop="amount" label="金额" align="center" min-width="40">
         ...
         <el-table-column prop="fundType" label="资金类型" align="center" min-width="40">
          <template slot-scope="scope">
            ...
            <span v-if ="scope.row.fundType==='welfare'">中秋福利</span>
            <span v-if ="scope.row.fundType==='gqWelfare'">国庆福利</span>
...
export default {
  ...
  methods: {
    ...
    // 查询
    searchHandle () {
      let url = '/store/account/fx/rewardData/queryFundList'
      if (this.channelCustFlag) {
        url = '/store/account/fx/fxChannelCust/queryFundList'
      }
      this.$ajax({
        url: url,
