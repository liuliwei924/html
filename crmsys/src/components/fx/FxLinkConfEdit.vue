  ...
  <el-dialog class="edit-dialog" title="添加链接" v-model="visible">
    <el-form :model="form" ref="form" label-width="150px">
      ...
      <el-form-item label="内容" prop="content" :rules="{required: true, message: '内容不能为空', trigger: 'blur'}">
        <el-input :rows="15" :cols="15" type="textarea" v-model="form.content" :maxlength="150" 
          placeholder="请输入帮助内容"></el-input>
    ...
    confirm () {
      this.$refs['form'].validate(valid => {
        if (valid) {
          ...
          this.$ajax({
            url: '/store/account/fx/fxLinkConf/updateLinkInfo',
            data: this.form,
            success: data => {
