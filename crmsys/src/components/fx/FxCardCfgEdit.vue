<template>
  <el-dialog class="edit-dialog" title="编辑卡种信息" v-model="visible">
    <el-form :model="form" ref="form" label-width="150px">
      ...
      <el-form-item label="产品名称" prop="prodName" :rules="{required: true, message: '产品名称不能为空', trigger: 'blur'}">
...
<script>
export default {
  name: 'fx-cardcfg-edit',
  props: {
    value: Boolean,
    fxCardcfgData: Object,
    prodList: Array
  },
  data () {
    return {
  ...
  methods: {
    ...
    confirm () {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          this.$ajax({
            url: '/store/account/fx/fxReferer/updateFxCardInfo',
            data: this.form,
            success: data => {
