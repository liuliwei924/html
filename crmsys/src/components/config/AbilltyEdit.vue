<template>
  <el-dialog class="edit-dialog" title="编辑等级配置" :visible.sync="visible">
    <el-form :model="form" ref="form" label-width="150px">
      <el-form-item label="能力等级ID" prop="gradeCode" :rules="{required: true, message: '能力等级ID不能为空', trigger: 'blur'}">
        <el-input v-model="form.gradeCode" placeholder="请输入能力等级ID" disabled></el-input>
      </el-form-item>
  ...
  methods: {
    ...
    confirm () {
      this.$refs['form'].validate(valid => {
        if (valid) {
          this.$ajax({
            url: '/store/account/config/abilityGrade/update',
            data: this.form,
            success: data => {
