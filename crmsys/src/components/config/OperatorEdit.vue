  ...
  <el-dialog
    ...
    <el-form
      ...
      <el-form-item class="multiple-form-item" label="管理门店">
        <el-checkbox v-model="isAll">管理所有门店</el-checkbox>
        <el-select
...
export default {
  ...
  methods: {
    ...
    // 保存处理
    saveHandle () {
      let userOrgs = this.form.userOrgs
      this.form.userOrgs = this.isAll ? 'all' : userOrgs.join(',')
      if (!this.authId && ['2', '4', '5', '12'].includes(this.form.roleId + '')) {
        this.form['authDealFlag'] = 1
      }
      this.$refs['form'].validate(valid => {
        if (valid) {
          this.$ajax({
            url: '/store/account/config/operator/checkUpdateInfo',
            ...
            fail: err => {
              ...
              this.form.userOrgs = this.form.userOrgs.split(',')
            }
          })
        }
      })
