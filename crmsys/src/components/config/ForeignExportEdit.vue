<template>
  <el-dialog class="edit-dialog" title="修改对外导出配置" v-model="visible">
...
<script>
export default {
  name: 'foreignExport-edit',
  props: {
    value: Boolean,
    externalUser: Array,
    foreignExportData: Object
  },
  data () {
    return {
      first: true,
  ...
  watch: {
    ...
    foreignExportData (obj) {
      ...
      this.form = {
        ...
        havePinan: obj['havePinan'] || '',
        fundType: obj['fundType'] || '',
        socialType: obj['socialType'] || '',
        wagesType: obj['wagesType'] || '',
        incomeData: obj['incomeData'] || '',
        exportTitile: obj['exportTitile'] || ''
      }
    }
  },
  methods: {
    ...
    confirm () {
      this.$refs['form'].validate(valid => {
        if (valid) {
          this.$ajax({
            url: '/store/account/config/foreignExportConf/updateForeginExport',
            data: this.form,
            success: data => {
