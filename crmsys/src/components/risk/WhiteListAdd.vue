<template>
  <el-dialog class="edit-dialog" title="添加白名单" :visible.sync="visible">
    <el-form :model="form" ref="form" label-width="150px">
      ...
      <el-form-item label="来源" prop="source" :rules="{required: true, message: '来源不能为空', trigger: 'blur'}">
        <el-select v-model="form.source" placeholder="请选择来源">
          <el-option v-for="item,key,index in source" :label="item" :value="key" :key="index"></el-option>
        </el-select>
      </el-form-item>
  ...
  methods: {
    ...
    confirm () {
      this.$refs['form'].validate(valid => {
        if (valid) {
          this.$ajax({
            url: '/crmsys/account/config/nameList/addWhite',
            data: this.form,
            success: data => {
