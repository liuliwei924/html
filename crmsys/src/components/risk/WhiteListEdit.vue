<template>
  <el-dialog class="edit-dialog" title="添加白名单" :visible.sync="visible">
    <el-form :model="form" ref="form" label-width="150px">
      ...
      <el-form-item label="来源" prop="source" :rules="{type: 'number', required: true, message: '来源不能为空', trigger: 'blur'}">
  ...
  methods: {
    ...
    confirm () {
      this.$refs['form'].validate(valid => {
        if (valid) {
          this.$ajax({
            url: '/crmsys/account/config/nameList/updateWhite',
            data: this.form,
            success: data => {
