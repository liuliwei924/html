...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('orgDealOrderDay')
    return {
      status: 1,
      signId: '',
      // 导出
      exportSearchParams: {},
      exportForm: {
        exportType: 'storeOrgDealOrderDay',
        exportParams: ''
      },
      defileName: 'orgDealOrderDay',
      searchForm: {
  ...
  methods: {
    ...
    // 查询总计求和
    queryOrgDealOrderDaySum () {
      this.sumTotalData = []
      // 缓存查询条件
      this.$ajax({
        url: '/store/account/dataset/orgDealOrderCount/queryOrgDealOrderDaySum',
    ...
    searchHandle () {
      ...
      // 缓存查询条件
      this.$setStore('orgDealOrderDay', this.searchForm, ['startRecordDate', 'endRecordDate'])
      this.$ajax({
        url: '/store/account/dataset/orgDealOrderCount/queryOrgDealOrderDay',
        data: this.searchForm,
        loading: true,
        success: data => {
