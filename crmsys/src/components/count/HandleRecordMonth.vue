...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('handleRecordMonth')
    return {
      status: 2,
      signId: '',
      // 导出
      exportSearchParams: {},
      exportForm: {
        exportType: 'storeHandleRecordMonth',
        exportParams: ''
      },
      defileName: 'handleRecordMonth',
      searchForm: {
  ...
  methods: {
    ...
    searchHandle () {
      ...
      // 缓存查询条件
      this.$setStore('handleRecordMonth', this.searchForm, ['startRecordMonth', 'endRecordMonth'])
      this.$ajax({
        url: '/store/account/dataset/handleRecordCount/queryHandRecordMonth',
        data: this.searchForm,
        loading: true,
        success: data => {
