...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('orderChannelDay')
    return {
      status: 1,
      signId: '',
      // 导出
      exportSearchParams: {},
      exportForm: {
        exportType: 'storeOrderChannelDay',
        exportParams: ''
      },
      defileName: 'orderChannelDay',
      searchForm: {
  ...
  methods: {
    ...
    // 查询总计求和
    queryOrderChannelDaySum () {
      this.sumTotalData = []
      // 缓存查询条件
      this.$ajax({
        url: '/store/account/dataset/orderChannelCount/queryOrderChannelDaySum',
    ...
    searchHandle () {
      ...
      // 缓存查询条件
      this.$setStore('orderChannelDay', this.searchForm, ['startRecordDate', 'endRecordDate'])
      this.$ajax({
        url: '/store/account/dataset/orderChannelCount/queryOrderChannelDay',
        data: this.searchForm,
        loading: true,
        success: data => {
