...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('orgDealOrderMonth')
    return {
      status: 2,
      signId: '',
      // 导出
      exportSearchParams: {},
      exportForm: {
        exportType: 'storeOrgDealOrderMonth',
        exportParams: ''
      },
      defileName: 'orgDealOrderMonth',
      searchForm: {
  ...
  methods: {
    ...
    // 查询总计求和
    queryOrgDealOrderMonthSum () {
      this.sumTotalData = []
      // 缓存查询条件
      this.$ajax({
        url: '/store/account/dataset/orgDealOrderCount/queryOrgDealOrderMonthSum',
    ...
    searchHandle () {
      ...
      this.exportSearchParams = this.searchForm
       // 缓存查询条件
      this.$setStore('orgDealOrderMonth', this.searchForm, ['startRecordMonth', 'endRecordMonth'])
      this.$ajax({
        url: '/store/account/dataset/orgDealOrderCount/queryOrgDealOrderMonth',
        data: this.searchForm,
        loading: true,
        success: data => {
      ...
      this.queryOrgDealOrderMonthSum()
    },
    // 分页处理
    pageHandle (page) {
