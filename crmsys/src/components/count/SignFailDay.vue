...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('signFailDay')
    return {
      status: 1,
      signId: '',
      // 导出
      exportSearchParams: {},
      exportForm: {
        exportType: 'storeSignFailDay',
        exportParams: ''
      },
      defileName: 'storeSignFailDay',
      searchForm: {
  ...
  methods: {
    ...
    querySignFailDaySum () {
      ...
      // 缓存查询条件
      this.$ajax({
        url: '/store/account/dataset/signFailCount/querySignFailDaySum',
    ...
    searchHandle () {
      ...
      // 缓存查询条件
      this.$setStore('signFailDay', this.searchForm, ['startRecordDate', 'endRecordDate'])
      this.$ajax({
        url: '/store/account/dataset/signFailCount/querySignFailDay',
        data: this.searchForm,
        loading: true,
        success: data => {
