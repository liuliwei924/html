...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('storeToDay')
    return {
      status: 3,
      signId: '',
      exportSearchParams: {},
      exportForm: {
        exportType: 'storePersonToDay',
        exportParams: ''
      },
      defileName: 'storePersonToDay',
      searchForm: {
  ...
  methods: {
    ...
    queryStoreToDayTotal () {
      ...
      // 缓存查询条件
      this.$ajax({
        url: '/store/account/dataset/storeCount/queryStoreToDayTotal',
    ...
    searchHandle () {
      ...
      // 缓存查询条件
      this.$setStore('storeToDay', this.searchForm)
      this.$ajax({
        url: '/store/account/dataset/storeCount/queryStoreToDay',
        data: this.searchForm,
        loading: true,
        success: data => {
