<template>
  <div class="content-box">
    ...
    <form :action="exportPath" method="post" id="JS-form-export" target="_blank" style="display:none">
          ...
          <input type="text" name="exportType" value="riskMonth">
    </form>
  </div>
</template>
...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('riskMonth')
    return {
      status: 2,
  ...
  methods: {
    getSummaries (param) {
      let nines = ['notHitCountSum', 'hitLeTimesCount3Sum', 'hitMoTimesCount3Sum', 'failCountSum', 'totalCountSum']
      let sums = ['总计']
      let arr = []
    ...
    queryRiskMonthSum () {
      ...
      // 缓存查询条件
      this.$ajax({
        url: '/store/account/dataset/riskCount/queryRiskMonthSum',
    ...
    searchHandle () {
      this.tableData = []
       // 缓存查询条件
      this.$setStore('riskMonth', this.searchForm, ['startRecordMonth', 'endRecordMonth'])
      this.$ajax({
        url: '/store/account/dataset/riskCount/queryRiskMonth',
        data: this.searchForm,
        loading: true,
        success: data => {
