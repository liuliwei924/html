...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('callMonth')
    return {
      status: 2,
      signId: '',
      // 导出
      exportSearchParams: {},
      exportForm: {
        exportType: 'storeCallMonth',
        exportParams: ''
      },
      defileName: 'callMonth',
      searchForm: {
        orgId: store['orgId'] || '',
  ...
  methods: {
    ...
    searchHandle () {
      ...
      // 缓存查询条件
      this.$setStore('callMonth', this.searchForm, ['startRecordDate', 'endRecordDate'])
      this.$ajax({
        url: '/store/account/dataset/queryDay',
