...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('queryRateMonth')
    return {
      status: 2,
      signId: '',
      // 导出
      exportSearchParams: {},
      exportForm: {
        exportType: 'storeOrderRateMonth',
        exportParams: ''
      },
      defileName: 'storeOrderRateMonth',
      searchForm: {
  ...
  methods: {
    ...
    queryRateDaySum () {
      ...
      // 缓存查询条件
      this.$ajax({
        url: '/store/account/dataset/orderRateCount/queryRateMonthSum',
    ...
    searchHandle () {
      ...
      this.exportSearchParams = this.searchForm
       // 缓存查询条件
      this.$setStore('queryRateMonth', this.searchForm, ['startRecordMonth', 'endRecordMonth'])
      this.$ajax({
        url: '/store/account/dataset/orderRateCount/queryRateMonth',
        data: this.searchForm,
        loading: true,
        success: data => {
