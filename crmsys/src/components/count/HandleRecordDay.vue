...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('handleRecordDay')
    return {
      status: 1,
      signId: '',
      // 导出
      exportSearchParams: {},
      exportForm: {
        exportType: 'storeHandleRecordDay',
        exportParams: ''
      },
      defileName: 'handleRecordDay',
      searchForm: {
  ...
  methods: {
    ...
    searchHandle () {
      ...
      // 缓存查询条件
      this.$setStore('handleRecordDay', this.searchForm, ['startRecordDate', 'endRecordDate'])
      this.$ajax({
