...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('signFailMonth')
    return {
      status: 2,
      signId: '',
      // 导出
      exportSearchParams: {},
      exportForm: {
        exportType: 'storeSignFailMonth',
        exportParams: ''
      },
      defileName: 'storeSignFailMonth',
      searchForm: {
  ...
  methods: {
    ...
    // 查询总计求和
    querySignFailMonthSum () {
      this.sumTotalData = []
      // 缓存查询条件
      this.$ajax({
        url: '/store/account/dataset/signFailCount/querySignFailMonthSum',
    ...
    searchHandle () {
      ...
      this.exportSearchParams = this.searchForm
       // 缓存查询条件
      this.$setStore('signFailMonth', this.searchForm, ['startRecordMonth', 'endRecordMonth'])
      this.$ajax({
        url: '/store/account/dataset/signFailCount/querySignFailMonth',
        data: this.searchForm,
        loading: true,
        success: data => {
