<template>
  <div class="content-box">
    ...
    <form :action="exportPath" method="post" id="JS-form-export" target="_blank" style="display:none">
          ...
          <input type="text" name="exportType" value="riskDay">
    </form>
  </div>
</template>
...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('riskDay')
    return {
      status: 1,
  ...
  methods: {
    ...
    queryRiskDaySum () {
      ...
      // 缓存查询条件
      this.$ajax({
        url: '/store/account/dataset/riskCount/queryRiskDaySum',
    ...
    searchHandle () {
      ...
      this.$ajax({
        url: '/store/account/dataset/riskCount/queryRiskDay',
        data: this.searchForm,
        loading: true,
        success: data => {
