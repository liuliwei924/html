...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('orgPauseReal')
    return {
      status: 1,
      signId: '',
      exportSearchParams: {},
      exportForm: {
        exportType: 'orgPauseReal',
        exportParams: ''
      },
      defileName: 'orgPauseReal',
      searchForm: {
  ...
  methods: {
    ...
    queryOrgTotal () {
      ...
      // 缓存查询条件
      this.$ajax({
        url: '/store/account/dataset/orgPauseCount/queryOrgPauseCountSum',
    ...
    searchHandle () {
      ...
      // 缓存查询条件
      this.$setStore('orgPauseReal', this.searchForm)
      this.$ajax({
        url: '/store/account/dataset/orgPauseCount/queryOrgPauseCount',
        data: this.searchForm,
        loading: true,
        success: data => {
