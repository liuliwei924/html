...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('orgToDay')
    return {
      status: 3,
      signId: '',
      // 导出
      exportSearchParams: {},
      exportForm: {
        exportType: 'storeOrgToDay',
        exportParams: ''
      },
      defileName: 'storeOrgToDay',
      searchForm: {
  ...
  methods: {
    ...
    queryOrgToDayTotal () {
      ...
      // 缓存查询条件
      this.$ajax({
        url: '/store/account/dataset/orgCount/queryOrgToDayTotal',
    ...
    searchHandle () {
      ...
      // 缓存查询条件
      this.$ajax({
        url: '/store/account/dataset/orgCount/queryOrgToDay',
        data: this.searchForm,
        loading: true,
        success: data => {
