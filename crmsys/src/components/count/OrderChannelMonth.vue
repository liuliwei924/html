...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('orderChannelMonth')
    return {
      status: 2,
      signId: '',
      // 导出
      exportSearchParams: {},
      exportForm: {
        exportType: 'storeOrderChannelMonth',
        exportParams: ''
      },
      defileName: 'orderChannelMonth',
      searchForm: {
  ...
  methods: {
    ...
    // 查询总计求和
    queryOrderChannelMonthSum () {
      this.sumTotalData = []
      // 缓存查询条件
      this.$ajax({
        url: '/store/account/dataset/orderChannelCount/queryOrderChannelMonthSum',
    ...
    searchHandle () {
      ...
      // 缓存查询条件
      this.$setStore('orderChannelMonth', this.searchForm, ['startRecordMonth', 'endRecordMonth'])
      this.$ajax({
        url: '/store/account/dataset/orderChannelCount/queryOrderChannelMonth',
        data: this.searchForm,
        loading: true,
        success: data => {
      ...
      this.queryOrderChannelMonthSum()
    },
    sortEvent (column) {
      if (column.prop === null) {
