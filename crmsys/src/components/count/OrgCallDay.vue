...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('StoreCallDay')
    return {
      status: 1,
      signId: '',
      // 导出
      exportSearchParams: {},
      exportForm: {
        exportType: 'storeOrgCallDay',
        exportParams: ''
      },
      defileName: 'orgCallDay',
      searchForm: {
        orgId: store['orgId'] || '',
  ...
  methods: {
    ...
    searchHandle () {
      ...
      // 缓存查询条件
      this.$setStore('StoreCallDay', this.searchForm, ['startRecordDate', 'endRecordDate'])
      this.$ajax({
        url: '/store/account/dataset/queryStoreCallDay',
