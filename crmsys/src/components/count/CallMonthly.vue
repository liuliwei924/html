...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('allMonthly')
    return {
      status: 3,
      signId: '',
      // 导出
      exportSearchParams: {},
      exportForm: {
        exportType: 'storeCallMonthly',
        exportParams: ''
      },
      defileName: 'callMonthly',
      searchForm: {
        orgId: store['orgId'] || '',
  ...
  methods: {
    ...
    searchHandle () {
      ...
      // 缓存查询条件
      this.$setStore('allMonthly', this.searchForm, ['startRecordMonth', 'endRecordMonth'])
      this.$ajax({
        url: '/store/account/dataset/queryMonth',
