...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('storeMonth')
    return {
      status: 2,
      signId: '',
      exportSearchParams: {},
      exportForm: {
        exportType: 'storePersonMonth',
        exportParams: ''
      },
      defileName: 'storePersonMonth',
      searchForm: {
  ...
  methods: {
    ...
    queryStoreMonthTotal () {
      ...
      // 缓存查询条件
      this.$ajax({
        url: '/store/account/dataset/storeCount/queryStoreMonthTotal',
    ...
    searchHandle () {
      ...
      // 缓存查询条件
      this.$setStore('storeMonth', this.searchForm, ['startRecordDate', 'endRecordDate'])
      this.$ajax({
        url: '/store/account/dataset/storeCount/queryStorePerMonth',
        data: this.searchForm,
        loading: true,
        success: data => {
