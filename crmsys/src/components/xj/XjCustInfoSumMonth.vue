...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('custInfoSumMonth')
    return {
      status: 2,
      signId: '',
      activeClass: 'active',
  ...
  methods: {
    ...
    searchHandle () {
      // 缓存查询条件
      this.$setStore('custInfoSumMonth', this.searchForm, ['startRecordDate', 'endRecordDate'])
      this.$ajax({
        url: '/store/account/xj/xjCustInfoSum/queryByMonth',
