<template>
  <el-dialog class="edit-dialog" title="添加奖励类型" v-model="visible">
    <el-form :model="form" ref="form" label-width="150px">
      ...
      <el-form-item
        ...
        prop="rewardValue"
        :rules="{required: false, message: '必填项', trigger: 'blur'}">
        <el-input v-model="form.rewardValue"></el-input>
      </el-form-item>
      <el-form-item
        label="奖励期限(天)"
        prop="rewardTerm"
        :rules="{required: false, message: '必填项', trigger: 'blur'}">
        <el-input v-model="form.rewardTerm"></el-input>
      </el-form-item>
      <el-form-item
        label="奖励类型"
        prop="rewardType"
  ...
  methods: {
    ...
    confirm () {
      this.$refs['form'].validate(valid => {
        if (valid) {
          this.$ajax({
            url: '/store/account/xj/xjTaskCfg/addTaskType',
            data: this.form,
            success: data => {
