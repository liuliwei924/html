...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('custSourceSumMonth')
    return {
      status: 3,
      signId: '',
      activeClass: 'active',
  ...
  methods: {
    ...
    searchHandle () {
      // 缓存查询条件
      this.$setStore('custSourceSumMonth', this.searchForm, ['startRecordDate', 'endRecordDate'])
      this.$ajax({
        url: '/store/account/xj/xjCustSourceSum/queryByMonth',
