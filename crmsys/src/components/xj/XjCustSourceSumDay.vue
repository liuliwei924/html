...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('custSourceSumDay')
    return {
      status: 2,
      signId: '',
      activeClass: 'active',
  ...
  methods: {
    ...
    searchHandle () {
      // 缓存查询条件
      this.$setStore('custSourceSumDay', this.searchForm, ['startRecordDate', 'endRecordDate'])
      this.$ajax({
        url: '/store/account/xj/xjCustSourceSum/queryThisMonth',
