...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('dayTaskSumMonth')
    return {
      status: 3,
      signId: '',
      activeClass: 'active',
      ...
      searchForm: {
        ...
        sourcetaskNameType: store['taskName'] || '',
  ...
  methods: {
    ...
    searchHandle () {
      // 缓存查询条件
      this.$setStore('dayTaskSumMonth', this.searchForm, ['startRecordDate', 'endRecordDate'])
      this.$ajax({
        url: '/store/account/xj/xjDayTaskSum/queryTaskSumMonth',
