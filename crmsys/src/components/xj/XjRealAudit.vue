<template>
  <el-dialog class="edit-dialog" title="审核实名信息" v-model="visible">
    <el-form :model="form" ref="form" label-width="150px">
      <el-form-item label="真实姓名" prop="realName" :rules="{required: false, message: '真实姓名不能为空'}">
        <el-input v-model="form.realName" placeholder="请输入真实姓名"></el-input>
      </el-form-item>
      <el-form-item label="身份证号" prop="showCardNo" :rules="{required: false, message: '身份证号不能为空'}">
        <el-input v-model="form.showCardNo" placeholder="请输入身份证号"></el-input>
      </el-form-item>
      <el-form-item label="人脸照片" prop="photoUrl">
        <template slot-scope="scope">
          <img :src="form.photoUrl" class="photoUrl" v-if="form.photoUrl" @click="alertBigImg(form.photoUrl)">
        </template>
      </el-form-item>
  ...
  methods: {
    ...
    confirm () {
      this.$refs['form'].validate(valid => {
        if (valid) {
          this.$ajax({
            url: '/store/account/xj/xjReal/checkCustRealName',
            data: {'customerId': this.form.customerId, 'status': this.form.status, 'realName': this.form.realName, 'cardNo': this.form.showCardNo},
            success: data => {
