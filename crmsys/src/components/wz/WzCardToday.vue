<template>
  <div class="content-box">
    ...
    <form :action="exportPath" method="post" id="JS-form-export" target="_blank" style="display:none">
          ...
          <input type="text" name="exportType" value="wzCardToday">
    </form>
  </div>
</template>
...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('wzCardToday')
    return {
      status: 3,
  ...
  methods: {
    ...
    queryWzCardTodaySum () {
      ...
      // 缓存查询条件
      this.$ajax({
        url: '/store/account/wz/wzCardCount/queryWzCardTodaySum',
    ...
    searchHandle () {
      this.tableData = []
      // 缓存查询条件
      this.$setStore('wzCardToday', this.searchForm, ['startRecordDate', 'endRecordDate'])
      this.$ajax({
        url: '/store/account/wz/wzCardCount/queryWzCardToday',
        data: this.searchForm,
        loading: true,
        success: data => {
