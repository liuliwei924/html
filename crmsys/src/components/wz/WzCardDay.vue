<template>
  <div class="content-box">
    ...
    <form :action="exportPath" method="post" id="JS-form-export" target="_blank" style="display:none">
          ...
          <input type="text" name="exportType" value="wzCardDay">
    </form>
  </div>
</template>
...
export default {
  ...
  data () {
    ...
    // 缓存查询条件
    let store = this.$getStore('wzCardDay')
    return {
      status: 1,
  ...
  methods: {
    ...
    queryWzCardDaySum () {
      ...
      // 缓存查询条件
      this.$ajax({
        url: '/store/account/wz/wzCardCount/queryWzCardDaySum',
    ...
    searchHandle () {
      // 缓存查询条件
      this.$setStore('wzCardDay', this.searchForm, ['startRecordDate', 'endRecordDate'])
      this.$ajax({
        url: '/store/account/wz/wzCardCount/queryWzCardDay',
        data: this.searchForm,
        loading: true,
        success: data => {
